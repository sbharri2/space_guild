...
const RESOURCE_TYPES = {
    ...
    Legal_1: {
        ...
        description: 'Common raw materials'
    },
    Legal_2: {
        category: 'Legal', 
        rarity: 2,
        basePrice: [80, 120],
        catchRate: 0,
        extractionEquipment: 'BasicMiningDrill',
        description: 'Uncommon processed materials'
    },
    Legal_3: {
        category: 'Legal',
        rarity: 3, 
        basePrice: [120, 180],
        catchRate: 0,
        extractionEquipment: 'AdvancedMiningDrill',
        description: 'Rare synthetic compounds'
    },
    Legal_4: {
        category: 'Legal',
        rarity: 4,
        basePrice: [180, 280],
        catchRate: 0,
        extractionEquipment: 'QuantumExtractor',
        description: 'Very rare quantum materials'
    },
    Legal_5: {
        category: 'Legal',
        rarity: 5,
        basePrice: [280, 450], 
        catchRate: 0,
        extractionEquipment: 'MolecularHarvester',
        description: 'Ultra rare exotic particles'
    },
    Legal_6: {
        category: 'Legal',
        rarity: 6,
        basePrice: [450, 800],
        catchRate: 0,
        extractionEquipment: 'ExoticMatterProcessor',
        description: 'Legendary metamaterials'
    },
    ...
    Illegal_1: {
        ...
        catchRate: 0.05,
        extractionEquipment: null,
        description: 'Low-risk contraband goods'
    },
    Illegal_2: {
        category: 'Illegal',
        rarity: 2,
        ...
        catchRate: 0.10,
        extractionEquipment: 'BasicMiningDrill',
        description: 'Restricted technology components'
    },
    Illegal_3: {
        category: 'Illegal',
        rarity: 3,
        ...
        catchRate: 0.20,
        extractionEquipment: 'AdvancedMiningDrill',
        description: 'Prohibited weapon materials'
    },
    Illegal_4: {
        category: 'Illegal',
        rarity: 4,
        ...
        catchRate: 0.35,
        extractionEquipment: 'QuantumExtractor',
        description: 'Dangerous experimental tech'
    },
    Illegal_5: {
        category: 'Illegal',
        rarity: 5,
        ...
        catchRate: 0.50,
        extractionEquipment: 'MolecularHarvester',
        description: 'Highly illegal alien artifacts'
    },
    Illegal_6: {
        category: 'Illegal',
        rarity: 6,
        ...
        catchRate: 0.70,
        extractionEquipment: 'ExoticMatterProcessor',
        description: 'Forbidden dimensional technology'
    }
};
...
const SPECIAL_SITE_TYPES = {
    ...
    DerelictShip: {
        ...
        resources: ['Legal_1', 'Legal_2', 'Illegal_1', 'Illegal_2'],
    ...
    RogueTrader: {
        ...
        resources: ['Legal_1', 'Legal_2', 'Legal_3', 'Illegal_1', 'Illegal_2'],
        extractableRatio: 0,
        tradableRatio: 1.0,
    ...
    AncientRuins: {
        ...
        resources: ['Illegal_4', 'Illegal_5', 'Illegal_6'],
    ...
    PirateCache: {
        ...
        resources: ['Illegal_1', 'Illegal_2', 'Illegal_3'],
        extractableRatio: 0,
        tradableRatio: 1.0,
        oneTime: true
    },
...
function resumeAnimations() {
    if (gameState.animation.isPaused && gameState.animation.animationsEnabled) {
        ...
        // Restart orbital animation if player is in a system
        const {x, y, z} = gameState.player.hexLocation;
        const knownSystem = Object.values(gameState.galaxy.knownSystems)
            .find(system => system.x === x && system.y === y && system.z === z);
        const genSystemId = `sys_${x}_${y}_${z}`;
...
function init() {
    ...
    // Initialize wormholes container if missing
...
function showHexStatusBox(hexId) {
    ...
    if ((isVisited || isClaimed) && hexResources && !systemData) {
        ...
        const actionsRow = `<div class="actions-row">${generateDriveButtons(col, row)}${scanButton}<button class="hex-action-btn">Mark</button><button class="hex-action-btn">Details</button></div>`;
        content = `
            <div class="status-box-header">
...
function tradeWithNpc(npcId) {
    const npc = (gameState.npc.ships || []).find(n => n.id === npcId);
    if (!npc) return;
    showGameDialog('Trade', 'Trading UI coming soon. This will be free to open.');
...
function createWormholeIcon(x, y) {
    return `<g class="wormhole-node">
              <circle cx="${x}" cy="${y}" r="10" fill="none" stroke="#66CCFF" stroke-width="2" opacity="0.9" />
              <circle cx="${x}" cy="${y}" r="6" fill="none" stroke="#66CCFF" stroke-width="1.5" opacity="0.8" />
              <circle cx="${x}" cy="${y}" r="2" fill="#66CCFF" opacity="0.9">
...
function renderWormholes(hexWidth, hexHeight) {
    ...
    for (const lane of gameState.galaxy.wormholes.lanes) {
        ...
        // Draw nodes only if visited
        if (visited.has(a)) g += createWormholeIcon(aPos.x, aPos.y);
        if (visited.has(b)) g += createWormholeIcon(bPos.x, bPos.y);

        // Show path if this lane is revealed or player is on a termination
        const showPath = (revealed === a || revealed === b || playerHexId === a || playerHexId === b);
        if (showPath && (visited.has(a) || visited.has(b))) {
